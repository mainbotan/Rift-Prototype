...
{
    public function getClients(array $data): ResultType 
    {
    ...
    public function createClient(array $data): ResultType 
    {
        return $this->buildInsertQuery($data)
            ->then(fn($stmt) => $this->executeQuery($stmt));
    }
    public function getClientByUid(string $uid): ResultType 
    {
        ...
        return $this->executeQuery($stmt)
            ->then(function($result) use ($uid) {
                if ($result[0] === null) {
                    return Result::Failure(Result::HTTP_NOT_FOUND, "Client {$uid} not found.");
                } 
                return Result::Success($result[0]);
            });
    }
    public function checkClientByUid(string $uid): ResultType 
    {
    ...
    public function deleteClientByUid(string $uid): ResultType 
    {
